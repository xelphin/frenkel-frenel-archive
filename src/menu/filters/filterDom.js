const FilterDom = (function FilterDom() {
    ...
    const makeInputId = (contentType, inputId) =>
        `${contentType}-item-input-${inputId}`;
    ...
    const createFilterItem = (infoObj, key, contentType) => {
        ...
        */
        ...
        filterItem.appendChild(
            createLabel(infoObj["label-text"], contentType, key),
        );
        let placeholder = infoObj["label-text"];
        if (Object.prototype.hasOwnProperty.call(infoObj, "placeholder"))
            placeholder = infoObj.placeholder;
        filterItem.appendChild(
            createInput(
                key,
                infoObj["label-text"],
    ...
    const initForm = (form, searchCallback) => {
        form.addEventListener("submit", (event) =>
            getSearchData(event, searchCallback),
        );
    };
