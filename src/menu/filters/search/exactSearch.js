...
const ExactSearch = (function ExactSearch() {
    ...
    const filterExactMatch = (allItems, databaseName, userInput) => {
        console.log(
            `Filtering: ${databaseName}, to match exact "${userInput}"`,
        );
        ...
        const allMatches = allItems.filter((item) => {
            ...
            itemValues = itemValues.map((str) =>
                SearchUtilities.cleanText(str),
            );
    ...
    const filterRangeExact = (
        ...
        console.log(
            `Filtering: ${databaseName}, by wanting above? ${wantAbove} the value ${userInput}, the type of the value is ${type}`,
        );
        // Filter
